...
def binarySearch(r: int, C: List[float]):
    ...
    while low <= high:
        ...
        # check mid - 1 < 0 -> mid[-1] ie the greatest node, not sure if this is inteded
...
def Edge_Select(G) -> Tuple[int, int]:
    def comulative_weights(D: List[float]) -> List[float]:
        Comulative: List[float] = []
        n_nodes = len(D)
    ...
    (D, W) = G
    C_D: List[float] = comulative_weights(D)
    # last one print("C_D: \t", C_D[-1])
    #  index start from 0 or 1?
    #  same for the v below
    u = Random_Select(C_D)  # + 1  # remove + 1 if needed
    print("u", u)
    C_W = comulative_weights(W[u - 1])  # To check
    # last one of vertex print("C_W: \t", C_W[-1])
    v = Random_Select(C_W)  # + 1  # remove + 1 if needed
    print("v", v)  # problem related to index starting 0 or 1 is the first? plus one to remove if needed
...
def Contract_Edge(G, u: int, v: int):
    ...
    D[u - 1] += D[v - 1] - (
        2 * W[u - 1, v - 1]
    )  # This is the only way to obtain a negative value
    D[v - 1] = 0
    W[u - 1, v - 1] = 0
    W[v - 1, u - 1] = 0
